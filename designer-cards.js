// Designer cards demo page — used only by designer_cards.html
// Requires: data/data.js, graduates.js (createGraduateCard)

const DEFAULT_DESIGNER_FULL_NAME = 'ADAM CHOO WEN WU';
const PROFILE_PAGE = 'designer_cards.html';
const OTHER_DESIGNERS_DESKTOP_COUNT = 5;
...
function initializeDesigner() {
    ...
    if (designerName) {
        ...
        currentDesignerIndex = findDesignerIndex(DEFAULT_DESIGNER_FULL_NAME);
        if (currentDesignerIndex < 0 && allGraduates.length > 0) {
            currentDesignerIndex = 0;
        }
        displayDesignerInfo(
            currentDesignerIndex >= 0
                ? getFullName(allGraduates[currentDesignerIndex])
                : DEFAULT_DESIGNER_FULL_NAME
        );
    }
...
function loadOtherDesigners() {
    ...
    const nextDesigners = getNextDesigners(OTHER_DESIGNERS_DESKTOP_COUNT);
    nextDesigners.forEach((graduate, index) => {
        const card = createGraduateCard(graduate, index);
        const fullName = getFullName(graduate);
        card.href = `${PROFILE_PAGE}?name=${encodeURIComponent(fullName)}`;
